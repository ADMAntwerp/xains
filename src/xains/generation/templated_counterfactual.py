"""TemplatedCounterfactualGenerator - LLM-free counterfactual narrative.

Consumes :func:`xains.counterfactuals.build_scenarios` to derive structured
per-scenario data, then renders one sentence describing the
counterfactual flip. Per ADR 0031 a request carries exactly one
counterfactual; the sentence leads with the flip and joins multiple
changed features with an Oxford-comma "and". Mirrors
:class:`xains.generation.templated.TemplatedNarrativeGenerator`'s shape
(ADR 0019) for the counterfactual path. See ADR 0030 / ADR 0031.

GenerationResult: ``text`` and ``latency_ms`` populated; ``prompt``,
``model_name``, ``raw_llm_response``, ``tokens_used``, and ``guardrails``
are all None (templated text has no LLM call to audit).

Tabular only.
"""

import time

from xains.config import ExplanationConfig
from xains.counterfactuals import build_scenarios
from xains.generation.base import GenerationResult, NarrativeGenerator
from xains.schema import DatasetSchema
from xains.types import ExplanationRequest, TabularExplanationRequest


class TemplatedCounterfactualGenerator(NarrativeGenerator):
    """Generate a counterfactual narrative from the pre-computed CF, no LLM.

    ``include_method=True`` appends ``" (method: <cf.method>)"`` to the
    sentence when the CF carries a non-``None`` ``method`` field. Default
    ``False`` keeps narratives method-agnostic, matching
    :class:`xains.prompts.counterfactual_tabular.CounterfactualTabularPromptTemplate`.
    """

    def __init__(self, *, include_method: bool = False) -> None:
        self._include_method = include_method

    def generate(
        self,
        request: ExplanationRequest,
        schema: DatasetSchema,
        config: ExplanationConfig,
    ) -> GenerationResult:
        if not isinstance(request, TabularExplanationRequest):
            raise TypeError(
                f"TemplatedCounterfactualGenerator requires a TabularExplanationRequest, "
                f"got {type(request).__name__}."
            )

        start = time.perf_counter()
        scenario = build_scenarios(request, schema)

        method_suffix = (
            f" (method: {scenario.method})"
            if self._include_method and scenario.method is not None
            else ""
        )

        if not scenario.changes:
            # Degenerate: no changed features (e.g. CF identical to factual).
            text = (
                f"To change the prediction from {scenario.factual_label} to "
                f"{scenario.cf_label}, no feature changes were detected{method_suffix}."
            )
        else:
            clauses = [
                f"{chg.name} would need to change from {chg.before} to {chg.after}"
                for chg in scenario.changes
            ]
            if len(clauses) == 1:
                joined = clauses[0]
            else:
                joined = ", ".join(clauses[:-1]) + ", and " + clauses[-1]
            text = (
                f"To change the prediction from {scenario.factual_label} to "
                f"{scenario.cf_label}, {joined}{method_suffix}."
            )

        latency_ms = (time.perf_counter() - start) * 1000.0

        return GenerationResult(
            text=text,
            prompt=None,
            model_name=None,
            raw_llm_response=None,
            tokens_used=None,
            latency_ms=latency_ms,
            guardrails=None,
        )
