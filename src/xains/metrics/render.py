...
and is omitted from the render. See ADR 0024.
"""
...
def render_grades(
    ...
    ``name: value``. Returns ``""`` when neither aggregate is supplied.
    """
    sections: list[str] = []
    if extraction is not None:
        sections.append(
            _render_section("Verbalization fidelity", extraction, EXTRACTION_GRADE_DIRECTIONS)
        )
    if narrativity is not None:
        sections.append(_render_section("Narrativity", narrativity, NARRATIVITY_GRADE_DIRECTIONS))
    return "\n\n".join(sections)


def _render_section(header: str, grades: BaseModel, directions: dict[str, str]) -> str:
    lines = [header]
