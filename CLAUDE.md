...
`xainarratives` is a **post-hoc explanation verbalizer**. It takes pre-computed inputs and produces natural-language explanations plus verbalization-quality metrics.
...
**Never import in core** (`src/xainarratives/` outside of `integrations/` and `eval/` subpackages):
...
These belong in optional extras, imported lazily inside `xainarratives/integrations/*` or `xainarratives/eval/*` adapters with `try: import x` guards that raise a clear `ImportError` telling the user the right `pip install xainarratives[extra]` command.
...
Adapters in `xainarratives/integrations/` are named by the **shape of the input**, not the upstream tool. E.g., `from_feature_importance` (accepts SHAP, LIME, sklearn feature_importances_, permutation importance, any signed-per-feature scalar), not `from_shap`.
